...
{
    public partial class Task17 : Form
    {
        ...
        private void Task17_Load(object sender, EventArgs e)
        {
        ...
        protected override void  OnPaint(PaintEventArgs e)
        {
            ...
            for (int i = 0; i < width; i++)
            {
                for (int j = 0; j < height; j++)
                {
                    if (i == 0)
                    {
                        g.DrawString(matrix.GetSumOfRow(j).ToString(), stringFont, Brushes.Black, width * GRID + OFFSET_X, j * GRID + OFFSET_Y);
                    }
                    g.DrawString(matrix[i][j].ToString(), stringFont, Brushes.Black, i * GRID + OFFSET_X, j * GRID + OFFSET_Y);
                }
                g.DrawString(matrix.GetSumOfCol(i).ToString(), stringFont, Brushes.Black, i * GRID + OFFSET_X, height * GRID + OFFSET_Y);
            }     
        }
